...
public class SelectElementProcessorBuilderEmbeddedUIContentController extends LeafNodeBuilderEmbeddedUIContentController<SelectElementProcessor> {
	public static final String FXML_FILE_DIR = "/builder/visframe/function/evaluator/sqlbased/utils/SelectElementProcessorBuilderEmbeddedUIContent.fxml";
	...
	protected void setupLogicToCheckEffectiveUIInput() {
		...
		this.stringValueTextField.focusedProperty().addListener((o,oldValue,newValue) -> {
			if(!this.stringValueTextField.focusedProperty().get()) {//focus is lost
				...
				////
				try {
					SelectElementProcessor processor = this.build();
...
//						Label tf = new Label(e);
	...
	@Override
	public SelectElementProcessorBuilder getOwnerNodeBuilder() {
		return (SelectElementProcessorBuilder) this.ownerNodeBuilder;
	}
	...
	@Override
	public SelectElementProcessor build() {
		
		if(this.stringValueTextField.getText().trim().isEmpty())
			throw new VisframeException("sql string cannot be empty!");
		
		try {
			SelectElementProcessor processor = new SelectElementProcessor(this.stringValueTextField.getText().trim());
...
//		
	...
	@Override
	public void setUIToNonNullValue(SelectElementProcessor value) {
		this.stringValueTextField.setText(value.getSingleLineSelectElementString());
